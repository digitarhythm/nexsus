...
// オブジェクトの動作を定義する
void enemy2( Puserparam usrprm, void *arg )
{
	unsigned short	tmp;
	short			i;
	...
	switch( usrprm -> processnum ){
		...
		case 2:
			if( NX_GetRandom( 1, 5 ) == 1 ){
				if( ( rand() % 2 ) == 0 ){
					usrprm -> pattern_num = 8;
					usrprm -> sp.sx = -1;
				} else {
					usrprm -> pattern_num = 9;
					usrprm -> sp.sx = 1;
				}
				NX_NextProcess( usrprm );
			}
		...
		case 3:
			usrprm -> sp.sx += ( 0.3 * ( usrprm -> sp.sx > 0 ) ) + ( -0.3 * ( usrprm -> sp.sx < 0 ) );
			if( usrprm -> loc.y > 320 || usrprm -> loc.x < -32 || usrprm -> loc.x > 240 ){
				tmp = NX_GetRandom( 40, 200 );
				NX_AddObject( tmp, -32, 0, 0, NX_GetRandom( 10, 20 ), 0, 0, NX_ANIME, usrprm -> num, 7, 1, 1, 1, 1, &( usrprm -> rect ), enemy2 );
				NX_DelObject( usrprm );
			}
			NX_EventAction( mcp, 99, ( void * )usrprm ); // 弾に当たったか
			NX_EventAction( mship, 99, ( void * )usrprm ); // 自機に当たったか
		...
		case 100:
				tmp = NX_GetRandom( 40, 200 );
				NX_AddObject( tmp, -32, 0, 0, NX_GetRandom( 10, 20 ), 0, 0, NX_ANIME, -1, 7, 1, 1, 1, 1, &( usrprm -> rect ), enemy2 );
				NX_DelObject( usrprm );
				break;
